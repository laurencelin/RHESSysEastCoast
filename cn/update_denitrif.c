...
//    double  theta, = patch[0].rootzone.S
    ...
    depth = min(patch[0].rootzone.depth, 1.5); //min(2.0, active_zone_z_sat);
    denitrify = 0.0;
    water_scalarSUM = 0.0;
    water_scalarCount = 0;
    fCO2 = 0.0;
	...
	if ( depth>0 && ns_soil->nitrate+patch[0].sat_NO3 > 0.0 && patch[0].soil_defaults[0][0].soil_water_cap>0.0 ) {
        ...
        if(z1>patch[0].rootzone.depth){
            ...
            if(fnitrate!=fnitrate || isinf(fnitrate)) printf("update_denitrif: z1>rtz [0-rtz] %e,%e,%e\n",layerZ,thickZ,tmpZ);
            fnitrate *= (patch[0].Ksat_vertical+(1.0-patch[0].Ksat_vertical)*0.05);
            perc_rtz += fnitrate;
            ...
            if(depth>patch[0].rootzone.depth){
                ...
                if(fnitrate!=fnitrate || isinf(fnitrate)) printf("update_denitrif: z1>rtz [rtz-z1] %e,%e,%e\n",layerZ,thickZ,tmpZ);
                fnitrate *= (patch[0].Ksat_vertical+(1.0-patch[0].Ksat_vertical)*0.05);
                perc_rtz += fnitrate;
                water_scalarSUM += water_scalar; water_scalarCount++;
            }//unsat
            ...
            if(depth>z1){
                ...
                fnitrate *= (patch[0].Ksat_vertical+(1.0-patch[0].Ksat_vertical)*0.05);
                if(fnitrate!=fnitrate || isinf(fnitrate)) printf("update_denitrif: z1>rtz [z1-depth] %e,%e,%e\n",layerZ,thickZ,tmpZ);
                
                if(denitrify>0 && resource_soilNO3+resource_satNO3>0){
                    tmp_ratio = resource_soilNO3/(resource_soilNO3+resource_satNO3);
                    perc_rtz += min(resource_soilNO3, fnitrate*tmp_ratio);
                    perc_sat += fnitrate - min(resource_soilNO3, fnitrate*tmp_ratio);
                }// partitioning
                water_scalarSUM += water_scalar; water_scalarCount++;
            }//satzone
            ...
            if(thickZ>0){
                ...
                if(fnitrate!=fnitrate || isinf(fnitrate)) printf("update_denitrif: z1<rtz [0-z1] %e,%e,%e\n",layerZ,thickZ,tmpZ);
                fnitrate *= (patch[0].Ksat_vertical+(1.0-patch[0].Ksat_vertical)*0.05);
                perc_rtz += fnitrate;
                water_scalarSUM += water_scalar; water_scalarCount++;
            }//rtz zone
            ...
            if(depth > z1){
                ...
                if(denitrify>0 && resource_soilNO3+resource_satNO3>0){
                    tmp_ratio = resource_soilNO3/(resource_soilNO3+resource_satNO3);
                    perc_rtz += min(resource_soilNO3, fnitrate*tmp_ratio);
                    perc_sat += denitrify - min(resource_soilNO3, fnitrate*tmp_ratio);
                }// partitioning
                water_scalarSUM += water_scalar; water_scalarCount++;
            }//satzone
        }
