...
void sort_patch_layers( struct patch_object *patch)
{
    ...
    // up to this point, the program tried to mark down different heights
	...
	for ( i=0 ; i<patch[0].num_layers ; i++ ){
		...
		for ( j=0 ; j<patch[0].num_canopy_strata; j++ ){
			...
			/*--------------------------------------------------------------*/
			if ( patch[0].canopy_strata[j][0].epv.height==(patch[0].layers[i]).height){  //<<------- this make different height plant exclusive
			/*--------------------------------------------------------------*/
