...
//    if(activedepthz > z1){
//        constantHold_no3 = (exp(-N_decay_rate*z1)-exp(-N_decay_rate*activedepthz)) / (1.0 - exp(-N_decay_rate*activedepthz));
...
//    if(activedepthz > z1){
//        constantHold_nh4 = (exp(-N_decay_rate*z1)-exp(-N_decay_rate*activedepthz)) / (1.0 - exp(-N_decay_rate*activedepthz));
...
//    if(activedepthz > z1){
//        constantHold_dom = (exp(-N_decay_rate*z1)-exp(-N_decay_rate*activedepthz)) / (1.0 - exp(-N_decay_rate*activedepthz));
...
//    }
	...
	for (k = 0; k < n_timesteps; k++) {
        ...
        //--------------------------------------------------------------------------- this is related to FC below
        patch[0].preday_sat_deficit_z = compute_z_final(verbose_flag,
                patch[0].soil_defaults[0][0].porosity_0,
                patch[0].soil_defaults[0][0].porosity_decay,
                patch[0].soil_defaults[0][0].soil_depth, 0.0,
                -1.0 * patch[0].sat_deficit);
        patch[0].preday_sat_deficit = patch[0].sat_deficit;  //<<--------------------- new: but why calculate every time step
        //---------------------------------------------------------------------------
		...
		for (i = 0; i < basin->route_list->num_patches; i++) {
			patch = basin->route_list->list[i];
            patch[0].hourly_subsur2stream_flow = 0;     ///<<<----------------------- hourly new
		...
		for (i = 0; i < basin->route_list->num_patches; i++) {
			...
			patch[0].sat_deficit += (patch[0].Qout - patch[0].Qin); // hourly
			patch[0].sat_deficit_z = compute_z_final(verbose_flag,
					patch[0].soil_defaults[0][0].porosity_0,
					patch[0].soil_defaults[0][0].porosity_decay,
					patch[0].soil_defaults[0][0].soil_depth, 0.0,
					-1.0 * patch[0].sat_deficit); //<<----- very important in my sat_solute code
...
//                patch[0].soil_ns.DON += (patch[0].soil_ns.DON_Qin - patch[0].soil_ns.DON_Qout);
			...
			if (patch[0].sat_deficit > patch[0].rootzone.potential_sat) {
				...
				patch[0].rootzone.S =
						min((patch[0].rz_storage + patch[0].rootzone.potential_sat - patch[0].sat_deficit) / patch[0].rootzone.potential_sat / (1.0-patch[0].basementFrac), 1.0);
				patch[0].S =
						min(patch[0].rz_storage / patch[0].sat_deficit, 1.0);
			}

			// we need to sort "sat_solute" into SAT and UNSAT here if wtz is dropping; 
			// we don't worry about wtz rising; it's been taken care of in the "update_drainage_land.c", right? the new material from upper layer will be added into "sat_solute" in the beginning of the next hour.
			...
			/*--------------------------------------------------------------*/
			//within a hourly loop; @ last hour within a day
			if(k ==(n_timesteps - 1)){// >=0, ==(n_timesteps - 1))
                ...
                // this is not smooth by daily step; that the "extra water" need to save for the next next day (not going to fix it)
			      ...
			      if(excess > 0) {
                        ...
                        if (grow_flag > 0) {
                            double N_decay_rate = (command_line[0].rootNdecayRate > 0? patch[0].rootzone.DOMdecayRate : patch[0].soil_defaults[0][0].DOM_decay_rate);
                            double activedepthz = (command_line[0].rootNdecayRate > 0? patch[0].soil_defaults[0][0].soil_depth : (command_line[0].root2active > 0.0? patch[0].rootzone.depth * command_line[0].root2active : patch[0].soil_defaults[0][0].active_zone_z));
                            Nout =
                                    compute_N_leached(
                                            verbose_flag,
                            ...
                            N_decay_rate = (command_line[0].rootNdecayRate > 0? patch[0].rootzone.NO3decayRate : patch[0].soil_defaults[0][0].N_decay_rate);
                            activedepthz = (command_line[0].rootNdecayRate > 0? patch[0].soil_defaults[0][0].soil_depth : (command_line[0].root2active > 0.0? patch[0].rootzone.depth * command_line[0].root2active : patch[0].soil_defaults[0][0].active_zone_z));
                            Nout =
                                    compute_N_leached(verbose_flag,
                                            patch[0].sat_NO3,
				...
				/*--------------------------------------------------------------*/
				//patch[0].overland_flow += patch[0].detention_store - patch[0].soil_defaults[0][0].detention_store_size; //overland_flow now is tracking return flow.
					...
					if (patch[0].drainage_type == STREAM) {
						...
						for (j = 0; j < patch->surface_innundation_list[d].num_neighbours; j++) {
							...
							if (neigh[0].drainage_type == STREAM) {
								...
								if (grow_flag > 0) {
									neigh[0].streamflow_DOC += (DOC_out * patch[0].area / neigh[0].area);
									neigh[0].streamflow_DON += (DON_out * patch[0].area / neigh[0].area);

									neigh[0].streamflow_NO3 += (NO3_out * patch[0].area / neigh[0].area);
									neigh[0].streamNO3_from_surface +=(NO3_out * patch[0].area / neigh[0].area);
									neigh[0].hourly[0].streamflow_NO3 += (NO3_out * patch[0].area / neigh[0].area);
									neigh[0].hourly[0].streamflow_NO3_from_sub +=(NO3_out * patch[0].area / neigh[0].area);

									neigh[0].streamflow_NH4 += (NH4_out * patch[0].area / neigh[0].area);
									neigh[0].surface_ns_leach += (Nout * patch[0].area / neigh[0].area);
								}
							} else {
								neigh[0].Qin_total += Qout * patch[0].area / neigh[0].area;
								neigh[0].detention_store += Qout * patch[0].area / neigh[0].area;
								if (grow_flag > 0) {
									neigh[0].surface_DOC += (DOC_out * patch[0].area / neigh[0].area);
									neigh[0].surface_DON += (DON_out * patch[0].area / neigh[0].area);
									neigh[0].surface_NO3 += (NO3_out * patch[0].area / neigh[0].area);
									neigh[0].surface_ns_leach -= (Nout * patch[0].area / neigh[0].area);
									neigh[0].surface_NH4 += (NH4_out * patch[0].area / neigh[0].area);
								}
							}
						}// end of for loop j
						if (grow_flag > 0) {
                ...
                //------------------------------------ this block of code here (below) looks very wrong
				/*--------------------------------------------------------------*/
				...
				if (patch[0].rootzone.depth > ZERO) {
					if ((patch[0].sat_deficit > ZERO) && (patch[0].rz_storage == 0.0)) {
						add_field_capacity = compute_layer_field_capacity(
								...
								patch[0].sat_deficit_z, patch[0].sat_deficit_z,
								0.0);
						add_field_capacity = max(add_field_capacity, 0.0)*(1.0-patch[0].basementFrac);
						patch[0].sat_deficit += add_field_capacity;
				...
				if (patch[0].sat_deficit < ZERO) {
					...
					/*-------------------------------------------------------*/
					patch[0].rootzone.S =
							min(patch[0].rz_storage / patch[0].rootzone.potential_sat/(1.0-patch[0].basementFrac), 1.0);
					rz_drainage = compute_unsat_zone_drainage(
							...
							0.5*patch[0].soil_defaults[0][0].Ksat_0_v * time_int, //<<-------------------- should be Ksat_0_v but keep the 1/2
							patch[0].rz_storage
									- patch[0].rootzone.field_capacity);
					...
					rz_drainage = compute_unsat_zone_drainage(
							...
							patch[0].S, patch[0].soil_defaults[0][0].mz_v,
							patch[0].sat_deficit_z,
							0.5*patch[0].soil_defaults[0][0].Ksat_0_v * time_int,
							patch[0].rz_storage
									- patch[0].rootzone.field_capacity);
