...
//    double tmpP;
//    int ii;
//    int currentJ = julday(start_date);
//    int currentJy = start_date.year;
//    tmin21ravg = (double *) alloc(duration*sizeof(double), "sequence","construct_clim_sequence");
//    daylestimate = (double *) alloc(duration*sizeof(double), "sequence","construct_clim_sequence");
//    dayl21ravg = (double *) alloc(duration*sizeof(double), "sequence","construct_clim_sequence");
//    for(ii=0; ii<duration; ii++){
//        if(ii<10 || ii+10>=duration){tmin21ravg[ii] = daily_clim[0].tmin[ii]; }
//        else{
//            tmin21ravg[ii] = (daily_clim[0].tmin[ii-10]+daily_clim[0].tmin[ii-9]+daily_clim[0].tmin[ii-8]+daily_clim[0].tmin[ii-7]+daily_clim[0].tmin[ii-6]+
//                              daily_clim[0].tmin[ii-5]+daily_clim[0].tmin[ii-4]+daily_clim[0].tmin[ii-3]+daily_clim[0].tmin[ii-2]+daily_clim[0].tmin[ii-1]+
//                              daily_clim[0].tmin[ii]+daily_clim[0].tmin[ii+1]+daily_clim[0].tmin[ii+2]+daily_clim[0].tmin[ii+3]+daily_clim[0].tmin[ii+4]+
//                              daily_clim[0].tmin[ii+5]+daily_clim[0].tmin[ii+6]+daily_clim[0].tmin[ii+7]+daily_clim[0].tmin[ii+8]+daily_clim[0].tmin[ii+9]+
//                              +daily_clim[0].tmin[ii+10])*0.04761905;
//        }
//        
//        tmpP = asin(.39795*cos(.2163108 + 2*atan(.9671396*tan(.00860*(currentJ-186)))));
//        daylestimate = 24 - 7.639437 * acos( (0.01454332 + basin[0].sin_latitude*sin(P))/(basin[0].cos_latitude*cos(P)) );
//        daylestimate *= 3600.0;
//        currentJ ++;
...
//    }//ii
    ...
    daily_clim[0].pretmin21ravg = NULL;
    daily_clim[0].predaylestimate = NULL;
    daily_clim[0].predayl21ravg = NULL;
