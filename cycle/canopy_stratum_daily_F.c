							   ...
							   struct date 			            current_date)
{
  ...
  /*--------------------------------------------------------------*/
	...
	stratum[0].gs_sunlit = compute_vascular_stratum_conductance(
		...
		stratum[0].ppfd_sunlit, // APAR
		stratum[0].epv.proj_lai_sunlit, //LAI
		stratum[0].epv.psi, // LWP_predawn (variable) for LWP
		zone[0].metv.tsoil, // Tair_min
		zone[0].metv.tday,  // Tair_mean_day
		zone[0].metv.vpd,   // VPD
        zone[0].CO2,
...
//                stratum[0].mult_conductance.vpd;
	...
	if ( (rnet_trans_sunlit > ZERO ) &&
		...
		potential_transpiration_rate_sunlit = penman_monteith(
			...
			zone[0].metv.vpd_day,
			rnet_trans_sunlit,
			1.0/stratum[0].potential_gs_sunlit,
			1.0/stratum[0].ga,
			2) ;
		}	
	else{
	...
	if ( (rnet_trans_shade > ZERO ) &&
		...
		potential_transpiration_rate_shade = penman_monteith(
			...
			zone[0].metv.vpd_day,
			rnet_trans_shade,
			1.0/stratum[0].potential_gs_shade,
			1.0/stratum[0].ga,
			2) ;
	}
	else{
	...
	if ( stratum[0].rootzone.depth > ZERO ){
		...
		if (Rnet_used > 0.0) {
			if ( (stratum[0].Lstar > 0) && (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux > 0) ){
                ...
                fraction_L_used = stratum[0].Lstar / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux);
				
                if (stratum[0].Lstar_night > 0.0) {
					fraction_L_used_night = stratum[0].Lstar_night / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux);
				}
                
				if (stratum[0].Lstar_day > 0.0) {
					fraction_L_used_day = stratum[0].Lstar_day / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux);
				}
                
				fraction_direct_K_used = stratum[0].Kstar_direct / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux);
				fraction_diffuse_K_used = stratum[0].Kstar_diffuse / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux);
				fraction_surfheat_used = stratum[0].surface_heat_flux / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].Lstar + stratum[0].surface_heat_flux);
			
            } else if ( stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].surface_heat_flux > 0.0 ) {
				fraction_direct_K_used = stratum[0].Kstar_direct / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].surface_heat_flux);
				fraction_diffuse_K_used = stratum[0].Kstar_diffuse / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].surface_heat_flux);
				fraction_surfheat_used = stratum[0].surface_heat_flux / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse + stratum[0].surface_heat_flux);
			}
			...
			APAR_used = (fraction_direct_K_used * Rnet_used + fraction_diffuse_K_used * Rnet_used) / (stratum[0].Kstar_direct + stratum[0].Kstar_diffuse) * (stratum[0].APAR_direct + stratum[0].APAR_diffuse);
		...
		deltaT = (Rnet_canopy * 1000 / 86400) /* energy in J/m2/s */
					...
					* (1.0/stratum[0].ga);   /* aerodynamic resistance in s/m */
	...
	if (stratum[0].defaults[0][0].lai_stomatal_fraction > ZERO ) {
		...
		if ((stratum[0].epv.all_lai > ZERO) && (stratum[0].snow_stored < ZERO))  {
			...
			if (stratum[0].defaults[0][0].epc.veg_type == C4GRASS)
				...
				psnin.c3 = 1;
			if (zone[0].metv.dayl > ZERO)
				psnin.Rd = stratum[0].cdf.leaf_day_mr / (stratum[0].epv.proj_lai * zone[0].metv.dayl*12.011e-9);
			else
				psnin.Rd = 0.0;
			...
			stratum[0].cdf.potential_psn_to_cpool = (assim_sunlit*stratum[0].epv.proj_lai_sunlit
					...
					*zone[0].metv.dayl*12.011e-9 + stratum[0].cdf.leaf_day_mr; //this potential is that 100% on soil water available
...
//                }//for layer
        ...
        if(stratum[0].phen.gwseasonday>0){
            ...
            //note
            // stratum[0].ppfd_sunlit = (stratum[0].APAR_direct + stratum[0].APAR_diffuse * perc_sunlit) / zone[0].metv.dayl /stratum[0].epv.proj_lai_sunlit;
            // stratum[0].ppfd_shade = stratum[0].APAR_diffuse * (1-perc_sunlit) / zone[0].metv.dayl /stratum[0].epv.proj_lai_shade;
            
            stratum[0].lFactor += (stratum[0].ppfd_sunlit*stratum[0].epv.proj_lai_sunlit + stratum[0].ppfd_shade*stratum[0].epv.proj_lai_shade);
//            if(stratum[0].cdf.potential_psn_to_cpool>0)
//                stratum[0].lFactor += stratum[0].cdf.psn_to_cpool/stratum[0].cdf.potential_psn_to_cpool;
        }//tracking
	...
	/*--------------------------------------------------------------*/
	patch[0].Kdown_direct_final += Kdown_direct * stratum[0].cover_fraction; // "Kdown_direct" was modified by "compute_diffuse_radiative_fluxes()" and "compute_direct_radiative_fluxes()"
    ...
    // "compute_diffuse_radiative_fluxes()" and "compute_direct_radiative_fluxes()"
    // look like these functions
    //  A) return the radiation used by canopy PSN and ET and store in stratum[0].APAR_direct, ... etc
    //  B) redistribute "Kdown_direct","Kup_direct","Kdown_diffuse","Kup_diffuse",... etc
	...
	patch[0].snow_throughfall_final += snow_throughfall * stratum[0].cover_fraction;
	patch[0].NO3_throughfall_final += NO3_throughfall * stratum[0].cover_fraction;
